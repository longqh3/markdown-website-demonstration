...
export const find = (arr, condition) => {
    return arr.filter(_ => {
        for (let key in condition) {
            if (Object.prototype.hasOwnProperty.call(condition, key) && condition[key] !== _[key]) {
                return false;
            }
        }
        return true;
...
export const findIndex = (arr, condition) => {
    ...
    arr.forEach((item, index) => {
        for (var key in condition) {
            if (Object.prototype.hasOwnProperty.call(condition, key) && condition[key] !== item[key]) { // 不进行深比较
                return false;
            }
        }
        ret = index;
